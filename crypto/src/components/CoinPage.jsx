    ...
    <div className='min-h-screen bg-gradient-to-br from-gray-900 via-gray-900/95 to-gray-900/90 text-white px-4 sm:px-[5%] md:px-[8%] py-6'>
      ...
      <div className='flex flex-col items-center md:flex-row gap-4 mb-6 bg-gray-800/30 backdrop-blur-lg p-4 rounded-xl border border-emerald-500/20'>
        <img src={coinDetails.image?.large} alt={coinDetails.name}
          className='w-16 h-16 md:w-20 md:h-20 rounded-full bg-gradient-to-br from bg-emerald-500/20 to-cyan-500/20 p-1' />
      ...
      <div className='space-y-3 md:hidden'>
        <StatCard title="Current Price" value={coinDetails.market_data.current_price[currentCurrency.name]} symbol={currentCurrency.symbol} />
        <StatCard title="Market Cap" value={coinDetails.market_data.market_cap[currentCurrency.name]} symbol={currentCurrency.symbol} />
        <div className='bg-gray-800/30 backdrop-blur-md p-4 rounded-lg border border-emerald-500/20'>
          <div className='space-y-3'>
            <PriceRange label="24h High" value={coinDetails.market_data.high_24h[currentCurrency.name]} symbol={currentCurrency.symbol} icon={<ArrowUp className='w-4 h-4 mr-1' />} color="text-green-400" />
            <PriceRange label="24h Low" value={coinDetails.market_data.low_24h[currentCurrency.name]} symbol={currentCurrency.symbol} icon={<ArrowDown className='w-4 h-4 mr-1' />} color="text-red-400" />
          </div>
        </div>
      </div>
      ...
      <div className='hidden md:grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6'>
        <StatBox title="Current Price" value={coinDetails.market_data.current_price[currentCurrency.name]} symbol={currentCurrency.symbol} />
        <StatBox title="Market Cap" value={coinDetails.market_data.market_cap[currentCurrency.name]} symbol={currentCurrency.symbol} />
        <div className='bg-gray-800/30 backdrop-blur-md p-4 rounded-xl border border-emerald-500/20'>
          ...
          <div className='flex justify-between items-center'>
            <PriceRange value={coinDetails.market_data.high_24h[currentCurrency.name]} symbol={currentCurrency.symbol} icon={<ArrowUp className='w-5 h-5 mr-1' />} color="text-green-400" />
            <PriceRange value={coinDetails.market_data.low_24h[currentCurrency.name]} symbol={currentCurrency.symbol} icon={<ArrowDown className='w-5 h-5 mr-1 text-red-400' />} color="text-red-400" />
          </div>
        </div>
      </div>

      {/* Last Row */}
      <div className='space-y-3 md:space-y-0 md:grid-cols-2 gap-4'>
        ...
        <div className='bg-gray-800/30 backdrop-blur-md p-4 rounded-lg md:rounded-xl border border-emerald-500/20'>
          <div className='flex justify-between items-center'>
            <span className='text-sm md:text-base text-emerald-400/90'>24h Volume</span>
            <span className='text-lg md:text-xl text-cyan-400'>
              {currentCurrency.symbol}{coinDetails.market_data.total_volume[currentCurrency.name].toLocaleString()}
            </span>
          </div>
        </div>
      </div>
...
const StatCard = ({ title, value, symbol }) => (
  <div className='bg-gray-800/30 backdrop-blur-md p-4 rounded-lg border border-emerald-500/20'>
    <div className='flex justify-between items-center'>
      <span className='text-sm text-cyan-400/80'>{title}</span>
...
const StatBox = ({ title, value, symbol }) => (
  <div className='bg-gray-800/30 backdrop-blur-md p-4 rounded-xl border border-emerald-500/20'>
    <h3 className='text-sm text-cyan-400/80 mb-2'>{title}</h3>
...
const PriceRange = ({ label, value, symbol, icon, color }) => (
